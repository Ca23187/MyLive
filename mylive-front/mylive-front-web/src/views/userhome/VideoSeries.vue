<template>
  <div class="video-list-panel">
    <div class="video-title">Playlists</div>
    <VueDraggable
    ...
    >
      <template v-for="(item, index) in videoSeriesList" :key="item.seriesId">
        ...
        >
          ...
          <div class="add-info">Create Playlist</div>
        </div>
        <div class="list-item" v-else @click="jump(item)">
          ...
          <div class="create-time">
            {{ proxy.Utils.formatDate(item.updatedAt) }}
          </div>
        </div>
      </template>
    </VueDraggable>
    <NoData v-if="videoSeriesList.length == 0" msg="No playlists yet"></NoData>
  </div>
  <VideoSeriesEdit
...
const rawVideoSeriesList = ref([]);

const videoSeriesList = computed({
  get() {
    if (myself.value) {
      return [
        {
        ...
        ...rawVideoSeriesList.value,
      ];
    }

    return rawVideoSeriesList.value;
  },

  set(list) {
    rawVideoSeriesList.value = list.filter((item) => {
...
const loadVideoSeries = async () => {
  ...
  rawVideoSeriesList.value = result.data || [];
};
...
const reorder = async () => {
  ...
  let seriesIds = rawVideoSeriesList.value.map((item) => {
    return item.seriesId;
  });

  let result = await proxy.Request({
    url: proxy.Api.uHomeSeriesReorderVideoSeries,
    params: {
  ...
  proxy.Message.success("Playlist order updated successfully");
};
