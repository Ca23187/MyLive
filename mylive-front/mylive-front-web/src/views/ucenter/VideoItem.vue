  ...
  <div class="video-item" ref="videoItemRef">
    ...
    <div class="video-info">
      <div class="video-name">
        <span v-if="data.status != 3">{{ data.videoTitle }}</span>
        <router-link
          v-else
          :to="`/video/${data.videoId}`"
          class="a-link"
          target="_blank"
          >{{ data.videoTitle }}</router-link
        >
        <span
      ...
      <div class="video-time">{{ proxy.Utils.formatDate(data.createdAt) }}</div>
        ...
        <div class="more-edit-panel">
          <div class="more-btn-list">
            ...
            <div class="interaction-op">
              <el-checkbox
                v-model="interactionInfo.danmaku"
                @change="saveInteractionInfo"
              >
                Disable Danmaku
              </el-checkbox>
              <el-checkbox
                v-model="interactionInfo.comment"
                @change="saveInteractionInfo"
              >
                Disable Comments
              </el-checkbox>
...
const interactionInfo = ref({
  danmaku: props.data.allowDanmaku === 0, // true = 关闭
  comment: props.data.allowComment === 0,
});
const saveInteractionInfo = async (e) => {
  let result = await proxy.Request({
    ...
    params: {
      ...
      allowDanmaku: interactionInfo.value.danmaku ? 0 : 1,
      allowComment: interactionInfo.value.comment ? 0 : 1,
    },
  });
  if (!result) {
    return;
...
const deleteVideo = () => {
  proxy.Confirm({
    message: `Are you sure you want to delete "${props.data.videoTitle}"?`,
    okfun: async () => {
      let result = await proxy.Request({
...
watch(
  ...
  (newVal) => {
    interactionInfo.value = {
      danmaku: newVal.allowDanmaku === 0,
      comment: newVal.allowComment === 0,
    };
  },
  { immediate: true }
);
