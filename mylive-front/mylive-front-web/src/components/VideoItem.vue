      ...
      <div class="cover" @click="showDetail">
        ...
        <div class="shade">
          <div class="play-count" v-show="layoutType == 0">
            ...
            <div class="iconfont icon-danmu">{{ data.danmakuCount }}</div>
          </div>
          <div class="play-time">{{ proxy.Utils.convertSecondsToHMS(data.duration) }}</div>
        </div>
      </div>
    </router-link>
    <div class="video-info">
      <!--高亮html展示-->
      <router-link class="title" :to="`/video/${data.videoId}`" v-html="data.videoTitle" target="_blank"></router-link>
      <router-link class="user-name" :to="`/user/${data.userId}`" target="_blank">
        <span class="iconfont icon-upzhu">{{ data.nickname }} · </span>
        <span>{{ proxy.Utils.formatDate(data.createdAt) }}</span>
      </router-link>
